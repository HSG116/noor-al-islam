...
export const fetchSurahs = async (): Promise<Surah[]> => {
  ...
  try {
    const local = localStorage.getItem('quran_surahs');
    if (local) {
      const parsed = JSON.parse(local);
      if (Array.isArray(parsed) && parsed.length > 0) {
        cachedSurahs = parsed;
        fetch(`${BASE_URL}/surah`).then(r => r.json()).then(d => {
          if (d?.data?.length) { cachedSurahs = d.data; try { localStorage.setItem('quran_surahs', JSON.stringify(d.data)); } catch {} }
        }).catch(() => {});
        return cachedSurahs;
      }
    }
  ...
  try {
    ...
    try { localStorage.setItem('quran_surahs', JSON.stringify(cachedSurahs)); } catch {}
    return cachedSurahs;
  } catch (error: any) {
